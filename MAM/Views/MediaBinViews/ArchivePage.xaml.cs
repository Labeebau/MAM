...
#pragma warning restore CA1416 // Validate platform compatibility
        ...
        private void LoadTreeView()
        {
            ...
            //string rootPath = GlobalClass.Instance.ArchivePath;
            //var rootNode = new TreeViewNode() { Content = rootPath };
            //FolderTreeView.RootNodes.Add(rootNode);
            //LoadSubfolders(rootNode, rootPath);
            //var newNode = FolderTreeView.RootNodes[0].Children[1].Children.FirstOrDefault(p => p.Content.Equals(archive.BinName));
            //if (!string.IsNullOrEmpty(archive.BinName) && newNode != null)
            //{
            ...
            //}
            // FolderTreeView.Expand(FolderTreeView.RootNodes[0].Children[1].Children.FirstOrDefault(p => p.Content.Equals(archive.BinName)));
        }
        ...
        public void GetAsset(string assetPath)
        {
            ...
            foreach (DataRow row in dt.Rows)
            {
                ...
                string thumbnailPath = System.IO.Path.Combine(archive.ProxyFolder, "Thumbnail_" + System.IO.Path.GetFileNameWithoutExtension(file) + ".jpg");
                string proxyPath = System.IO.Path.Combine(archive.ProxyFolder, "Proxy_" + (file));
            ...
            archive.FileCount = MediaPlayerItems.Count;
        ...
        private async void TreeView_SelectionChanged(TreeView sender, TreeViewSelectionChangedEventArgs args)
        {
            ...
            if (selectedNode != null)
            {
                ...
                if(dirName == ((FileSystemItem)FileTreeView.SelectedNode.Content).Name)
                    archive.BinName =((FileSystemItem)FileTreeView.SelectedNode.Content).Path; 
                else
                    archive.BinName =Path.Combine(((FileSystemItem)FileTreeView.SelectedNode.Content).Path, ((FileSystemItem)FileTreeView.SelectedNode.Content).Name);

                //archive.BinName = GetFullPath(FileTreeView.SelectedNode);
        ...
        public async Task DeleteMediaItemAsync(MediaPlayerItem item)
        {
            ...
            if (result == ContentDialogResult.Primary)
            {
                ...
                if (MediaPlayerItems.Contains(item))
                {
                    ...
                    try
                    {
                        File.Delete(System.IO.Path.Combine(archive.BinName, item.Title));
                        File.Delete(System.IO.Path.Combine(archive.BinName, item.ThumbnailPath.LocalPath));
                        File.Delete(System.IO.Path.Combine(archive.BinName, item.ProxyPath.LocalPath));
                        DeleteAssetAsync(item.Title,archive.BinName);
                    }
        ...
        private async void AddAsset_Click(object sender, RoutedEventArgs e)
        {
            ...
            //archive.BinName = GetFullPath(FileTreeView.SelectedNode);
                ...
                archive.BinName = string.Empty;
            if (!(string.IsNullOrEmpty(archive.BinName) || Equals(archive.BinName, FileTreeView.RootNodes[0].Content.ToString()) ||
                Equals(System.IO.Path.GetFileNameWithoutExtension(archive.BinName), FileTreeView.RootNodes[0].Children[0].Content.ToString()) ||
                Equals(System.IO.Path.GetFileNameWithoutExtension(archive.BinName), FileTreeView.RootNodes[0].Children[1].Content.ToString())))
            {
        ...
        private async void AddNewBin_Click(object sender, RoutedEventArgs e)
        {
            if (FileTreeView.SelectedNode == null)
                archive.BinName = string.Empty;
            if (!(string.IsNullOrEmpty(archive.BinName) || System.IO.Path.Equals(archive.BinName, FileTreeView.RootNodes[0].Content.ToString()) || System.IO.Path.Equals(archive.BinName, FileTreeView.RootNodes[0].Children[0].Content.ToString())))
